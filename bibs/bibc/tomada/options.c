...
int opt_meth_setoption(lua_State *L, p_opt opt, p_socket ps)
{
    ...
    if (!opt->func) {
        const char *msg = lua_pushfstring(L,
            "op\xC3\xA7\xC3\xA3o n\xC3\xA3o aceita `%s'", name);
...
int opt_set_linger(lua_State *L, p_socket ps)
{
    ...
    lua_pushstring(L, "ativo");
    lua_gettable(L, 3);
    if (!lua_isboolean(L, -1))
        luaL_argerror(L, 3, "campo booleano 'ativo' esperado");
    li.l_onoff = (u_short) lua_toboolean(L, -1);
    lua_pushstring(L, "tempoEsgotado");
    lua_gettable(L, 3);
    if (!lua_isnumber(L, -1))
        luaL_argerror(L, 3, "campo num\xC3\xA9rico 'tempoEsgotado' esperado");
...
int opt_get_linger(lua_State *L, p_socket ps)
{
    ...
    lua_setfield(L, -2, "ativo");
    lua_pushinteger(L, li.l_linger);
    lua_setfield(L, -2, "tempoEsgotado");
    return 1;
}
...
int opt_get_ip_multicast_if(lua_State *L, p_socket ps)
{
    ...
    if (getsockopt(*ps, IPPROTO_IP, IP_MULTICAST_IF, (char *) &val, &len) < 0) {
        lua_pushnil(L);
        lua_pushstring(L, "falha ao obter op\xC3\xA7\xC3\xA3o da tomada");
        return 2;
    }
...
static int opt_setmembership(lua_State *L, p_socket ps, int level, int name)
{
    ...
    if (!lua_istable(L, 3)) auxiliar_typeerror(L,3,lua_typename(L, LUA_TTABLE));
    lua_pushstring(L, "endere\xC3\xA7oMulticast");
    lua_gettable(L, 3);
    if (!lua_isstring(L, -1))
        luaL_argerror(L, 3, "campo colar 'endere\xC3\xA7oMulticast' esperado");
    if (!inet_aton(lua_tostring(L, -1), &val.imr_multiaddr))
        luaL_argerror(L, 3, "campo 'endere\xC3\xA7oMulticast' cont\xC3\xA9m IP inv\xC3\xA1lido");
    lua_pushstring(L, "interface");
    lua_gettable(L, 3);
    if (!lua_isstring(L, -1))
        luaL_argerror(L, 3, "campo colar 'interface' esperado");
    val.imr_interface.s_addr = htonl(INADDR_ANY);
    if (strcmp(lua_tostring(L, -1), "*") &&
            !inet_aton(lua_tostring(L, -1), &val.imr_interface))
        luaL_argerror(L, 3, "endere\xC3\xA7o IP 'interface' inv\xC3\xA1lido");
...
static int opt_ip6_setmembership(lua_State *L, p_socket ps, int level, int name)
{
    ...
    if (!lua_isnil(L, -1)) {
        if (lua_isnumber(L, -1)) {
          ...
          luaL_argerror(L, -1, "campo num\xC3\xA9rico 'interface' esperado");
    }
    return opt_set(L, ps, level, name, (char *) &val, sizeof(val));
}
...
int opt_set(lua_State *L, p_socket ps, int level, int name, void *val, int len)
{
    if (setsockopt(*ps, level, name, (char *) val, len) < 0) {
        lua_pushnil(L);
        lua_pushstring(L, "falha ao definir op\xC3\xA7\xC3\xA3o da tomada");
        return 2;
    }
    lua_pushnumber(L, 1);
    return 1;
