/* Exceções finalizadas herdadas do LuaSocket. */
#include "luasocket.h"
#include "except.h"
...
static luaL_Reg func[] = {
    {"verifique", global_newtry},
    {"proteja",   global_protect},
    {NULL,         NULL}
};
...
static int protected_(lua_State *L) {
    ...
    lua_pushvalue(L, lua_upvalueindex(2));
    lua_insert(L, 1);
    status = lua_pcallk(L, lua_gettop(L) - 1, LUA_MULTRET, 0, 0,
        protected_cont);
...
int except_open(lua_State *L) {
    lua_newtable(L);
    lua_pushboolean(L, 0);
    lua_setfield(L, -2, LUA_MM_METATABELA);
